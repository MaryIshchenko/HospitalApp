...
export class Location extends Component{
    ...
    FilterFn(){
        ...
        var filteredData=this.state.LocationsWithoutFilter.filter(
            function(el){
        ...
        this.setState({Location:filteredData});

    }

    sortResult(prop,asc){
        var sortedData=this.state.LocationsWithoutFilter.sort(function(a,b){
            if(asc){
        ...
        this.setState({Location:sortedData});
    }
    ...
    refreshList(){
        fetch(variables.API_URL+'Location')
        .then(response=>response.json())
        .then(data=>{
            this.setState({Location:data,LocationsWithoutFilter:data});
        });
    }
    ...
    deleteClick(id){
        ...
        fetch(variables.API_URL+'Location/'+id,{
            method:'DELETE',
            headers:{
                'Accept':'application/json',
                'Content-Type':'application/json'
...
<div>
    ...
    <tbody>
        {Location.map(dep=>
            <tr key={dep.LocationId}>
                <td>{dep.LocationId}</td>
                <td>{dep.LocationName}</td>
                <td>
                <button type="button"
                ...
                <button type="button"
                className="btn btn-light mr-1"
                onClick={()=>this.deleteClick(dep.LocationId)}>
...
<div className="modal-content">
   ...
   <div className="modal-body">
       <div className="input-group mb-3">
        <span className="input-group-text">LocationName</span>
